...
const CODE_EXTENSIONS = ['.html', '.tsx', '.ts', '.css', '.json', '.js', '.jsx'];
...
async function optimizeImages() {
    ...
    for (const imagePath of images) {
        ...
        try {
            ...
            console.log(`Optimized: ${path.basename(imagePath)} -> ${(saved / 1024).toFixed(2)} KB saved`);
...
async function updateCodeReferences() {
    ...
    for (const codeFile of codeFiles) {
        ...
        for (const oldRelativePath of processedFiles) {
            ...
            const newBasename = oldBasename.replace(path.extname(oldBasename), '.webp'); // image.webp
        ...
        if (modified) {
            ...
            console.log(`Updated references in: ${path.relative(ROOT_DIR, codeFile)}`);
        }
    }
}
